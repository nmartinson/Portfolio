...
class CreateGallery extends React.Component {
  ...
  uploadFiles(){
    const { galleryName, files, isChecked} = this.state;
    const apiUrl = process.env.API_URL;
    const path = `${apiUrl}/gallery`

    var images = files.map((fileItem, index) => {
      return { file: fileItem.file, name: fileItem.inputName, uniqueFileName: fileItem.name, settings: fileItem.settings, isFeatured: isChecked }
    });
